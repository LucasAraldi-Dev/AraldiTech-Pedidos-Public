...
## [0.8.8] - 2025-05-23
### Adicionado
- **Dashboard**: Foi refeito completamente , dividido em menus para melhor organização.
- **Relatórios**: Criado um sistema no backend especifico para relatórios, porém ainda em desenvolvimento.

### Corrigido
- **Responsividade** : Novamente foi detectado problemas com a responsividade , foram feitos os ajustes necessários.



## [0.8.7] - 2025-05-21
### Adicionado
- **UI Interativa**: Criado alguns componentes para a UI-UX para os usuários
- **Confirmação Logout**: Criado um modal para confirmar o logout para evitar cliques por engano.
- **Menu Burguer**: Versão mobile foi implementado o menu burguer no Home do App
- **Splash Screen**: Agora tem uma melhor UX com uma tela de carregamento para preparar demais componentes do sistema , sem que o usuário perceba.

### Melhorado
- **AppLogin**: Foi refeito algumas funcionalidades e etapas após o login.
- **Modal de Cadastro**: Otimizado validação dos campos e informações ao usuário.

### Corrigido
- **Responsividade**: Depois de algumas atualizações de layout foi necessário fazer o ajuste e centralizar a parte de responsividade em um arquivo separado.
- **Modal de Cadastro**: Era possivel colocar o espaço no nome do usuário , e ao usar o auto-preenchimento foi observado que as vezes colocava espaço ao final do nome de usuário.
- **Verificação de token**: Algumas rotas foi retirado a necessidade de token , porém foi adicionada uma camada de segurança para evitar problemas futuros.

## [0.8.6] - 2025-05-19
### Melhorado
- **Responsividade**: Responsividade no geral foi melhorada , principalmente para telas pequenas e mobile.
- **Botões e Menus**: Alguns foram refeitos para apresentar melhor consistência na versão mobile.

### Corrigido
- **Visualizador de logs**: Correção nas informações presentes do modal de logs.
- **ModalImprimirPedido**: Adaptação da opção de gerar imagem , para caber totalmente o form para sair a impressão correta.


## [0.8.5] - 2025-05-18
### Adicionado
- **Segurança Aprimorada**: Implementação de proteção CSRF em todos os endpoints, com validação avançada de dados e proteção contra XSS e injeção SQL.
- **Sistema de Cache**: Mecanismo de cache para requisições frequentes, reduzindo o tempo de resposta e a carga no servidor.
- **Visualizador de Logs do Sistema**: Novo modal para administradores acompanharem atividades do sistema com filtros avançados e exportação.
- **Configuração de Testes Unitários**: Framework Jest integrado para testes automatizados de componentes e serviços.
- **Serviço de Validação Centralizado**: Implementação de regras de validação padronizadas para uso em todos os formulários.
- **Componentes de UI Reutilizáveis**: Novos componentes para indicadores de carregamento e tooltips informativos.
- **Tratamento Avançado de Erros**: Sistema centralizado para tratamento de erros de API com feedback contextual.

### Melhorado
- **Performance da Aplicação**: Otimização das requisições HTTP com sistema de cache e redução de chamadas desnecessárias.
- **Feedback ao Usuário**: Notificações mais claras e indicação visual de operações em andamento.
- **Responsividade**: Ajustes nos layouts de modais e cards para melhor visualização em diferentes dispositivos.
- **Segurança de Dados**: Validação rigorosa de entradas do usuário para prevenir ataques comuns.
- **Organização do Código**: Refatoração para melhor modularidade e padrões de desenvolvimento.
- **Consistência Visual**: Padronização dos componentes visuais em toda a aplicação.
- **Visualização de Logs**: Melhor categorização dos logs e organização para futuras auditorias.

### Corrigido
- **Inconsistências nos Formulários**: Uniformização da validação e mensagens de erro entre todos os formulários.
- **Problemas de Responsividade**: Correção de layout em telas específicas, especialmente em dispositivos móveis.
- **Tratamento de Datas**: Formato consistente de datas entre formulários e relatórios.
- **Concorrência de Requisições**: Melhor gerenciamento de múltiplas requisições simultâneas.
- **Comportamento em Rede Instável**: Recuperação mais robusta de falhas temporárias de conexão.
- **Conclusão dos Pedidos**: Correção relacionada ao formato da data e permissões de acesso a botão de concluir.

### Removido
- **Tutorial Interativo**: O sistema de tutorial passo a passo para novos usuários foi removido. As informações de uso agora estão disponíveis na documentação.
